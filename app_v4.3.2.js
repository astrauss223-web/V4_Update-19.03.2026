        ...
        funds.forEach(fund => {
            ...
            if (inpEinmal) {
                inpEinmal.addEventListener('blur', e => {
                    const v = parseEuro(e.target.value);
                    if (v > 0) { portfolioGlobals.fundInvestments[key] = v; e.target.value = formatNumberInput(v); }
                    else { delete portfolioGlobals.fundInvestments[key]; e.target.value = ''; }
                    saveGlobals();
                    updateFinancialOverview(); renderPanel();
                });
            ...
            if (inpSpar) {
                inpSpar.addEventListener('blur', e => {
                    const v = parseEuro(e.target.value);
                    if (v > 0) { portfolioGlobals.fundSparrates[key] = v; e.target.value = formatNumberInput(v); }
                    else { delete portfolioGlobals.fundSparrates[key]; e.target.value = ''; }
                    saveGlobals();
                    updateFinancialOverview(); renderPanel();
                });
